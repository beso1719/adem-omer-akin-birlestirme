...
// Profil resimleri (img/p0.png ... p10.png). 11 resim, 11 boyut → birebir.
const PROFILE_COUNT = 11;
for (let i = 0; i < FRUITS.length; i++) {
  const img = new Image();
  img.onload = () => { try { drawNext(); } catch (e) {} };
  img.src = `img/p${i % PROFILE_COUNT}.png`;
  FRUITS[i].img = img;
}
...
function fitCanvas() {
  ...
  canvas.width = W;
  canvas.height = H;
  S = W / 720;            // referans genişlik → daha küçük toplar, çok daha uzun oyun
  DROP_Y = H * 0.07;
  DEATH_Y = H * 0.12;
}
fitCanvas();
...
let gameOver = false;
let dropX = W / 2;
const effects = [];     // birleşme halkaları
const particles = [];   // birleşme parçacıkları
const floaters = [];    // uçan puan yazıları

// ---- Combo (kısa sürede arka arkaya birleşme) ----
let combo = 0;
let comboTimer = 0;
...
Events.on(engine, "collisionStart", (e) => {
  for (const pair of e.pairs) {
    ...
    if (a.tier >= MAX_TIER) {          // en büyük + en büyük → ikisi de yok
      a.merging = b.merging = true;
      addEffect(mx, my, MAX_TIER);
      burst(mx, my, FRUITS[MAX_TIER].color, 22);
    ...
    const nf = makeFruit(newTier, mx, my, false);
    World.add(world, nf);
    addEffect(mx, my, newTier);
    burst(mx, my, FRUITS[newTier].color, 12);
    onMerge(mx, my, (newTier + 1) * 2);
  }
});

// Birleşme olunca: combo say, puanı combo ile çarp, yazı + ses
function onMerge(x, y, base) {
  comboTimer = 0.9;
  combo++;
  const mult = combo >= 2 ? combo : 1;
  const gained = base * mult;
  addScore(gained);
  floaters.push({ x, y, text: "+" + gained, t: 0, combo: mult > 1 ? mult : 0 });
  pop(combo);
}
...
function burst(x, y, color, n) {
  for (let i = 0; i < n; i++) {
    ...
    particles.push({
      ...
      r: (2 + Math.random() * 3) * S,
      color, t: 0, life: 0.5 + Math.random() * 0.4,
    });
  }
}
...
muteBtn.addEventListener("click", () => {
  muted = !muted;
  localStorage.setItem("meyve_muted", muted ? "1" : "0");
  muteBtn.textContent = muted ? "🔇" : "🔊";
});
...
function restart() {
  ...
  effects.length = 0; particles.length = 0; floaters.length = 0;
  nextTier = randTier();
...
function render(dt = 0.016) {
  ...
  // nişan çizgisi (bekleyen top nereye düşecek)
  if (current && canDrop && !gameOver) {
    ...
    ctx.setLineDash([6, 8]);
    ctx.beginPath();
    ctx.moveTo(x, current.position.y + current.radius);
    ctx.lineTo(x, H);
    ctx.stroke();
    ctx.setLineDash([]);
    ctx.restore();
  ...
  for (let i = floaters.length - 1; i >= 0; i--) {
    const fl = floaters[i];
    fl.t += dt;
    if (fl.t >= 1) { floaters.splice(i, 1); continue; }
    const a = 1 - fl.t;
    ctx.save();
    ctx.globalAlpha = a;
    ctx.textAlign = "center";
    ...
    ctx.fillStyle = fl.combo ? "#ffd33a" : "#fff";
    const fs = (fl.combo ? 26 : 20) * S;
    ctx.font = `800 ${fs}px "Segoe UI", sans-serif`;
    const y = fl.y - fl.t * 40 * S;
    const txt = fl.combo ? `${fl.text}  COMBO x${fl.combo}` : fl.text;
    ctx.strokeText(txt, fl.x, y);
    ctx.fillText(txt, fl.x, y);
    ctx.restore();
  }
}
